...
4. 架构风格：
   ...
   | 架构风格                 | 子风格        | 构件                                          | 连接件                           | 主要特点                                                     |
   | :----------------------- | :------------ | :-------------------------------------------- | :------------------------------- | :----------------------------------------------------------- |
   | **数据流**               | 批处理        | 独立程序                                      | 某种类型的媒介（如文件）         | **传统编译器风格**，整体传输；顺序执行，前一步完全完成后才进入下一步。 |
   |                          | 管道-过滤器   | 过滤器                                        | 管道                             | **每个阶段产生的结果作为下一阶段的输入**；管道负责传输，过滤器负责处理；支持并行。 |
   | **调用/返回**            | 面向对象      | 对象                                          | 交互方式（如方法调用、消息传递） | 通过对象间交互实现功能。                                     |
   |                          | 层次结构      | 各层模块                                      | 层间协议或调用接口               | 系统组织为层次，每层为上层提供服务。                         |
   |                          | 主程序/子程序 | 主程序和子程序                                | 过程调用                         | 典型的主程序调用子程序结构。                                 |
   | **独立构件**             | 进程通信      | 独立的进程                                    | 消息传递                         | 独立进程间通过消息传递通信。                                 |
   |                          | 事件驱动      | 事件处理器                                    | 事件触发机制                     | 由事件触发相应动作，如**语法高亮**等。                       |
   | **虚拟机**               | 解释器        | 解释器组件                                    | 解释执行连接                     | 用户自定义更灵活。                                           |
   |                          | 规则系统      | 规则集、规则解释器、规则/数据选择器、工作内存 | 规则匹配与执行                   | 将业务逻辑中频繁变化的部分定义为规则。定义关系进行执行。**专家系统** |
   | **仓库**（以数据为中心） | 数据库系统    | 中央数据结构、独立构件                        | 数据共享访问                     | 典型如**IDE**，**数据共享**为主。                            |
   |                          | 超文本        | 节点、链接                                    | 超链接                           | 通过超链接组织数据。                                         |
   |                          | 黑板          | 黑板（中央数据结构）、知识源（独立构件）      | 黑板读写机制                     | 用于**语音识别、知识推理**；**中央数据结构**说明当前数据的状态，**独立构件**在中央数据存储上执行。 |
   | **闭环**                 | 过程控制      | 控制器、传感器、执行器                        | 反馈控制回路                     | **设定参数，如定速巡航**。                                   |
   | **C2**                   | C2            | 并行构件                                      | 连接件（按规则绑定）             | 通过连接件绑定在一起并按照一组规则运作的并行构件。           |

5. C/S架构：三层在两层上增加一个**应用服务器**，构成包括**表示层**(用户接口部分，存在于客户机)、**功能层**(应用本体)和**数据层**(数据库管理系统)三个部分
...
8. 特定领域软件架构(DSSA) 通常是一个具有三个层次的系统模型，包括**领域开发环境、领域特定应用开发环境和应用执行环境**。
   ...
   **领域架构师**在领域开发环境工作，**应用工程师**在领域特定的应用开发环境工作，**操作员**在应用执行环境工作。
   ...
   4角色：**领域专家**(提供需求规格和实现的知识)、**领域分析师**(获得的知识组织到**领域模型**中)、**领域设计人员**(根据领域模型和现有系统开发出 **DSSA** 以及建立两者联系)和**领域实现人员**(软件实现，可重用构件验证)
...
10. 软件架构复用：类型：**机会复用**(抓住机会)和**系统复用**(开发前规划)；主要阶段：构造/获取可复用的软件资产、管理可复用资产和使用可复用资产

11. 架构权衡分析方法(ATAM): 4阶段：需求收集、**架构视图**描述、属性模型构造和分析、架构决策与折中；强调以**属性**为架构评估的核心概念，主要针对性能、可用性、安全性和可修改性，在系统开发之前，对这些质量属性进行**评价和折中**。通过头脑风暴和优先场景理解质量属性要求，分析架构方法的三种场景：用例场景，增长情景，探索性情景。

12. 基于场景的架构分析方法(SAAM)：针对最终架构而非详细设计进行评估。5步骤：场景开发、体系结构描述、单个场景评估、场景交互和总体评估。主要输入问题是**问题描述、需求声明和体系结构描述**。
...
20. 数字孪生体有三种类型：**数字孪生原型体、数字孪生实例体和数字孪生聚合体**。数字孪生环境则是数字孪生体的操作环境。
...
26. 软件配置管理(SCM)核心内容包括**版本控制和变更控制**。SCM活动的目标就是为了标识变更、控制变更、确保变更正确实现并向其他有关人员报告变更。

27. 净室软件工程的理论基础主要是**函数理论和抽样理论**。

28. 维护分为4种类型：**排错性维护、适应性维护、完善性维护和预防性维护**。
...
5. 数据库也分为三级模式，分别是**外模式、概念模式和内模式**。

6. 关系的完整性约束共分为3类：**实体完整性、参照完整性(引用完整性)和用户定义完整性**。

7. 数据库设计分为**需求分析、概念结构设计、逻辑结构设计、物理结构设计、应用程序设计和运行维护**6个阶段。

8. NoSQL整体框架分为4层，由下至上：**数据持久层、数据分布层、数据逻辑模型层和接口层**。
...
OSI七层：物理层、数据链路层、网络层、传输层、会话层、表示层和应用层。
...
2. 测试内容划分：功能测试、性能测试、用户界面测试和安全性测试
   ...
   Alpha测试是在**软件开发环境**下由用户进行的测试；Beta测试是在**实际环境**中由多个用户对其进行测试。

   白盒测试方法：控制流分析、数据流分析、路径分析、程序变异等。覆盖程度：语句覆盖、判定覆盖、分支覆盖和路径覆盖。
...
3. 企业资源规划 ERP 5层次计划：经营规划、销售与运作规划 (生产规划)、主生产计划、物料需求计划、车间作业控制
   ...
   ERP中的企业资源包括企业的&#34;三流&#34;资源，即**物流资源、资金流资源和信息流资源**。
...
| 数据文件大小 | 较AOF小                                                      | 记录内容越多，文件越大，数据恢复变慢                         |

3. Redis高可用机制
   - 主从复制：主节点负责处理写请求，从节点负责处理读请求。主节点将写操作记录到日志中，从节点通过日志复制主节点的数据。当主节点发生故障时，从节点可以提升为主节点继续提供服务。
   - 哨兵模式：哨兵节点监控主节点和从节点的状态，当主节点发生故障时，哨兵节点会从从节点中选举出一个新的主节点。
   - 集群模式：集群模式将数据分片存储在多个节点上，每个节点负责一部分数据。集群模式支持数据的自动分片和故障转移，提高了系统的可用性和扩展性。
...
9. 常见术语
   ...
   DAO(数据库访问对象)位于业务逻辑层与数据资源层之间。


### 应用设计

1. MVC设计模式 (架构风格)
   * 模型(Model)：业务逻辑处理和数据存储。
   * 视图(View)：展示数据。
   * 控制器(Controller)：接受用户请求，调用模型处理数据请求，返回视图。
...
2. J2EE平台：
   ...
   * 客户层
   * web层： 与浏览器通过HTTP交互
     * JSP：动态网页技术，组织信息内容并为用户产生响应页面的HTML数据流。
     * Servlet：处理HTTP请求，用来接收用户输入信息，执行业务操作逻辑后转发用户请求。
   * 业务层： EJB(Enterprise JavaBean)组件，用于封装业务逻辑，实现业务功能
   * 企业信息系统层： ERP/数据库
     * JDBC：Java连接数据库的标准接口(执行SQL)
     * JPA：对象关系映射框架，用Java对象操作数据库(无需写SQL)

3. kafaka: 消息中间件，分布式流处理平台，用于构建实时数据管道和流应用程序。它具有高吞吐量、可扩展性和持久性等特点，适用于处理大规模数据流。日志收集流式处理，事件驱动架构等场景。
   - producer：生产者，负责将数据发送到kafka集群。
   - customer：消费者，负责从kafka集群中读取数据。
   - broker：kafka集群中的服务器节点，负责存储和管理消息。
   - topic：主题，用于存储消息的类别。
   - partition：分区，用于将消息分成多个部分，提高并发处理能力。
   - offset：偏移量，用于标识消息在分区中的位置。
   - zookeeper：用于管理kafka集群的元数据，如broker列表、topic列表等。
         新版本 Kafka 逐步移除对 Zookeeper 的依赖(KRaft模式)
   - 高可用，高拓展性
      * 分区副本机制(每个分区有多个副本(Leader &#43; Follower)，Leader 负责读写，Follower 同步数据)
      * 消费者组
      * ISR(In-Sync Replicas)等机制。
   - 消息投递语义
      * at most once：最多一次，消息可能丢失，但不会重复。
      * at least once：最少一次，消息不会丢失，但可能重复。
      * exactly once：恰好一次，消息不会丢失或重复。

4. 常见术语
   
   ORM(Object Relational Mapping)：对象关系映射，将对象和数据库表进行映射，通过操作对象来操作数据库表。典型ORM框架：Hibernate、MyBatis(持久层框架)、JPA。

   ES(elasticsearch):开源分布式搜索和分析引擎，在存储层可用于存储索引数据，方便对数据进行快速检索。

   k8s(Kubernetes):开源的容器编排平台，用于自动化部署、扩展和管理容器化应用程序。

   CDN: 内容分发网络。主要加速静态资源，HTML、CSS和JS。

   REST(Representational State Transfer，表述性状态转移)。

   JWT(Json Web Token用于在双方之间安全传输信息的简洁的、URL安全的令牌标准)。
