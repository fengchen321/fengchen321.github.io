...
7. `ctrl y`: 粘贴使用前三种查出方式的文本。（误删的时候派上用场）
...
5. `man sh`：查看 `sh`（Bourne shell）的手册页
   ...
   &gt;
   &gt; `man -a kill`：查看 `kill` 命令的所有手册页，适合查看同名命令的不同版本或不同功能的手册页。 1: 用户命令；2: 系统调用；3: 库函数；4: 设备文件；5: 文件格式；6: 游戏；7: 杂项；8: 管理命令
